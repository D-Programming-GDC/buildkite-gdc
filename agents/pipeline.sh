...
for target in "${targets[@]}"; do
    ...
    if [ "${native_ports[$name]:-x}" != 'x' ]; then
        if [ `expr "${BUILDKITE_BRANCH}" : '.*master-ci'` -ne 0 ]; then
            host='native'
        elif [ `expr "${BUILDKITE_PULL_REQUEST_BASE_BRANCH}" : '.*master-ci'` -ne 0 ]; then
            host='native'
        else
            host='cross'
        fi
        ...
        if [ `expr "${BUILDKITE_BRANCH}" : '.*master-ci'` -eq 0 ]; then
            bootstrap='enable'
        elif [ `expr "${BUILDKITE_PULL_REQUEST_BASE_BRANCH}" : '.*master-ci'` -eq 0 ]; then
            bootstrap='enable'
        fi
        host='bootstrap'
    else
